...
<body>
                ...
                if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    ...
                    if (mysqli_num_rows($result) > 0) {
                        ...
                        if (isset($_POST["update"])) {
                            $cname = $_POST["c_name"];
                            $courseid = $_POST["c_id"];
                            $unitcode = $_POST["u_code"];
                            $school = $_POST["school"];

                            // Update the user's information in the database
                            $sql = "UPDATE courses SET c_name = '".$cname."', course_id = '".$courseid."',unit_code = '".$unitcode."', school = '".$school."' WHERE id = '".$id."'";
    ...
    <form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>">
        ...
        <input type="text" name="c_id" value="<?php echo $courseid; ?>"><br>
        <label for="email">Unit Code:</label>
        <input type="text" name="u_code" value="<?php echo $unitcode; ?>"><br>
        <label for="name">  School:</label>
        <input type="text" name="school" value="<?php echo $school; ?>"><br>
        <input type="submit" value="Update Changes" class="editting" name="update">
    </form>
